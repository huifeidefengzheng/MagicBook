...
object EtlProcess {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //2、读取数据
    val source: DataFrame = spark.read.json("data/data.json")
    //3、处理数据
    //3.1  列裁剪，只取出ip字段
    source.createOrReplaceTempView("t_source")
    val ips = source.selectExpr("ip")
    //3.2  去重，根据ip去重
    val distinctIp = ips.distinct()
    //3.3  解析Ip，获取经纬度、省份、城市
